...
class SalesDetailPage extends StatelessWidget {
  ...
  const SalesDetailPage({required this.sales, Key? key}) : super(key: key);

  Future<void> _deleteSales(BuildContext context) async {
          ...
          children: <Widget>[
            Container(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: <Widget>[
                  Text('Pembeli: ${sales.buyer}',
                      style: const TextStyle(fontSize: 18)),
                  const SizedBox(height: 8),
                  Text('Telepon: ${sales.phone}',
                      style: const TextStyle(fontSize: 18)),
                  const SizedBox(height: 8),
                  Text('Tanggal: ${sales.date}',
                      style: const TextStyle(fontSize: 18)),
                  const SizedBox(height: 8),
                  Text('Status: ${sales.status}',
                      style: const TextStyle(fontSize: 18)),
                  const SizedBox(height: 8),
                  Text('Vendor: ${sales.issuer}',
                      style: const TextStyle(fontSize: 18)),
                  const SizedBox(height: 8),
                  Text('Dibuat: ${sales.createdAt}',
                      style: const TextStyle(fontSize: 18)),
                  const SizedBox(height: 8),
                  Text('Diperbarui: ${sales.updatedAt}',
                      style: const TextStyle(fontSize: 18)),
                ],
              ),
            ),
            ...
            Row(
              ...
              children: <Widget>[
                ElevatedButton.icon(
                  onPressed: () async {
                    final isUpdated = await Navigator.push<bool>(
                      context,
                      MaterialPageRoute(
                          builder: (context) => EditSalesPage(sales: sales)),
                    );
                    if (isUpdated == true) {
                ...
                ElevatedButton.icon(
                  onPressed: () => _deleteSales(context),
